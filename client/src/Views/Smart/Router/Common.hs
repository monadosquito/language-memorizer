...
import Views.Smart.SignIn.Common (signIn)
import Views.Smart.SignUp.Common (signUp)
...
router model = either (const . home $ BemClass "ActivePage" [ darkMode' model ] []) id
  ...
  where
    ...
    pages
       ...
       :<|> signIn (BemClass "ActivePage" [ darkMode' model ] [])
       :<|> signUp (BemClass "ActivePage" [ darkMode' model ] [])
